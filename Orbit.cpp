...
int main() {
  ...
  if (outdata.fail()) {
    cout << " Unable to open" << filename << "--- Terminating calculation"
         << "\n\n"
  ...
  cout << "     Orbit computes the orbit of a small mass about a much larger "
          "mass.\n"
       << endl;
  ...
  do {
    // Calculate the distance from the principal focus using Eq. (2.3); Kepler's
    // First Law.
    ...
    double dfdy = y - prevy;
    double deltat = t-tprev;
    double velo = sqrt(pow((dfdx / deltat), 2) + pow((dfdy / deltat), 2));
    
    KE = (0.5) * mustar * pow(velo, 2);
    cout << KE << endl;
    tprev = t;
    }
    // If time to print, convert to cartesian coordinates.  Be sure to
    // print last point also.
    if (k == 1) {
      outdata << fixed << showpoint << right << setprecision(3) << setw(14)
              << t / yr << setw(18) << r / AU << setw(23)
              << theta * radians_to_degrees << setw(18) << r * cos(theta) / AU
              << setw(18) << r * sin(theta) / AU << setw(18) << scientific << PE << setw(18)
              << scientific << KE << endl;
    }
    ...
    k++;
    if (k > kmax || (theta - two_pi) / two_pi > delta || (t - P) / P > delta)
      k = 1;
  } while ((theta - two_pi) < dtheta / 2 && (t - P) < dt / 2);

  cout << "\n\nThe calculation is finished and the data are in" << filename
       << "\n\n"
       << endl;
